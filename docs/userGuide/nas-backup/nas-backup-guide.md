...
!!! info
     백업 트래픽은 KVM 호스트와 백업 저장소 간 직접 통신으로 처리되므로 별도의 백업 전용 대역을 사용할 수 있습니다. 백업 대상 호스트는 해당 대역으로 라우팅 가능해야하며, 백업 저장소 등록 시 백업 전용 네트워크의 NAS 주소를 사용해야 합니다. 호스트는 백업 수행 시점에 NAS를 직접 마운트하므로 사전 수동 마운트는 필요하지 않습니다.
