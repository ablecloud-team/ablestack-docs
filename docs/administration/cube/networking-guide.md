...
1. 네트워킹 메뉴를 클릭합니다.
2. 제거할 본드를 클릭하십시오.
3. 본드 설정 화면에서 ON/OFF 버튼으로 본드를 비활성화 하거나 삭제 버튼을 클릭하여 본드를 영구적으로 제거 할 수 있습니다.
...
5. 선택적으로 STP(Spanning Tree 프로토콜) 를 선택하여 브릿지 루프 및 브로드 캐스트 방사를 방지 할 수 있습니다.
    - 강한 선호도가 없는 경우 사전정의 된 값을 그대로 두십시오.
6. 적용 버튼을 클릭합니다.
...
3. 인터페이스 세부 정보 화면에서 IPv4 구성을 클릭합니다.
...
!!! warning
    Cube의 웹 콘솔 포트는 현재 고정되어 있으며, 변경이 필요한 경우 엔지니어가 작업을 진행해야합니다.
