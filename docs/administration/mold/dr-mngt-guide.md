...
이 메뉴는 Mold가 설치된 경우 기본적으로 표시되지 않으며, 재난 복구 클러스터 메뉴를 표시하려면 Mold 관리자는 전역 구성 매개 변수 cloud.dr.service.enabled 를 "true"로 설정해야 합니다.
...
1. 인프라스트럭쳐 > 재난 복구 클러스터 메뉴로 이동하여 설정을 변경할 클러스터의 상세화면에서 **설정** 탭을 클릭합니다.
    * Primary 클러스터의 경우 Secondary 클러스터 Mold의 API 키 & 비밀 키, Glue의 비밀키 와 미러링 스냅샷 스케줄 interval 항목을 변경할 수 있습니다.
...
!!! check
    미러링 가상머신을 생성하기 위해 Secondary 클러스터의 Mold 에 사용자 지정 크기가 활성화되고, 공유 볼륨이 비활성화된 디스크 오퍼링이 존재해야 합니다.
!!! warning
...
1. Primary 클러스터에서 컴퓨트 > 가상머신 메뉴로 이동하여 미러링을 해제 할 가상머신의 상세화면의 **DR** 탭에서 **미러링 해제** 버튼을 클릭하고, 확인 창에서 **예** 버튼을 클릭하여 미러링을 해지합니다.
...
2. DR 클러스터 동기화
    ...
    !!! info
        Primary 클러스터를 복구한 경우 강제 디모트 작업 실행 전 복구된 Primary 클러스터의 이미지를 동기화하는 작업을 실행합니다.
...
3. DR 클러스터 강제 디모트
    ...
    !!! warning
        동기화 명령은 완료되었지만, 미러링 이미지의 용량마다 syncing 작업 완료 시간이 다를 수 있으며 모든 이미지들이 정상적으로 완료되기 전 강제 디모트 기능 실행 시 실패할 수 있습니다.

4. Primary 클러스터 확인
    - Primary 클러스터의 Mold 에서 미러링한 가상머신 구동
    !!! warning
        복구가 완료된 후 재난 복구 클러스터의 미러링 에이전트 상태가 Warning 이나 Error 인 경우 재난 복구 비활성화 또는 재난 복구 클러스터 삭제하여 재구성해야 합니다. 
