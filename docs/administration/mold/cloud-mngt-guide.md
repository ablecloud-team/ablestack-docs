...
1. SNMP 관리자의 경우 SNMP 관리자 시스템에 CloudStack MIB 파일을 설치합니다. 이것은 SNMP OID를 사용자가 더 쉽게 읽을 수있는 트랩 유형에 매핑합니다. 파일은 공개적으로 사용 가능해야합니다. 이 파일을 설치하는 방법에 대한 자세한 내용은 SNMP 관리자와 함께 제공된 설명서를 참조하십시오.
