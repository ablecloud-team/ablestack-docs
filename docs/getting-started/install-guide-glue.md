...
!!! note
    ABLESTACK HCI 클러스터는 최소 3식이상의 호스트로 구성되어야 합니다.
...
1. 라이선스 등록
    ![라이선스 등록](../assets/images/install-guide-glue-license1-01.png){ align=center }
    - 구축 전, 발급 받은 **트라이얼 라이선스 파일** 을 등록 해줍니다.

2. 라이선스 확인
    ![라이선스 확인](../assets/images/install-guide-glue-license1-02.png){ align=center }
...
!!! Tip
    + 클러스터로 시간 서버가 정해진 후에 호스트의 시간이 작업자 콘솔의 시간과(사용자 콘솔의 시간과) 일치하는지 확인합니다. 그렇지 않으면 Wall 구성시 모니터링 정보를 제대로 불러오지 못할 수 있습니다. 시간이 제대로 설정되었다면 hwclock -w 명령어로 하드웨어에 해당 시간을 입력합니다.
    + 시간을 맞춘 후에 각 호스트에서 chronyc sources 명령어로 시간 서버가 제대로 설정되었는지 확인합니다.
...
2. 가상머신 장치 구성 - 컴퓨트
    ...
    !!! tip
        ...
        를 선택하시면 됩니다.
        스토리지 가상머신의 ROOT Disk **70GiB** 는 디스크가 **Thin Provisioning** 방식으로 제공합합니다.

3. 가상머신 장치구성 - 디스크

    !!! info
        <br>스토리지 센터 가상머신은 스토리지 클러스터를 구성하여 디스크를 생성합니다. 구성 방법에 따라 Raid-Passthrough와 LUN-Passthrough를 선택하게 됩니다.</br>
        + Raid-Passthrough는 Raid 카드를 스토리지 센터 가성머신에 할당하는 경우입니다. 해당 항목을 선택하면 Raid 카드 목록이 출력됩니다.
        + LUN-Passthrough는 이미 구성된 LUN을 스토리지 센터 가상머신에 할당하는 경우입니다. 해당 항목을 선택하면 LUN 목록이 출력됩니다.
    ...
    - **디스크 구성 방식** 은 **LUN Passthrough** 를 선택합니다.
    - **디스크 구성 대상 장치** 로는 **Glue 스토리지로 사용할 디스크** 를 선택하고 **다음** 버튼을 클릭 합니다.
...
1. 라이선스 등록
    ![라이선스 등록](../assets/images/install-guide-glue-license2-01.png){ align=center }
    - 구축 전, 발급 받은 **트라이얼 라이선스 파일** 을 등록 해줍니다.

2. 라이선스 확인
    ![라이선스 확인](../assets/images/install-guide-glue-license2-02.png){ align=center }
    - 등록된 라이선스 정보를 확인할 수 있는 화면입니다.
...
3. 가상머신 장치구성 - 디스크
   ![가상머신 장치 구성 - 컴퓨트](../assets/images/install-guide-glue-26.png){ align=center }
    - 스토리지 센터 가상머신 장치의 디스크 구성 화면입니다.
    - **디스크 구성 방식** 은 **LUN Passthrough** 를 선택하고, **Glue 스토리지로 사용할 디스크** 를 선택한 후에 **다음** 버튼을 클릭 합니다.

    !!! warning
        가상머신 장치 구성 - 디스크 구성시 파티션이 구성된 디스크의 선택을 하시면 안됩니다.
        만약에 파티션이 구성된 디스크를 선택을 하고 가상 머신을 구성하시면 ABLESTACK Cube 가 삭제될 수 있습니다.
...
1. 라이선스 등록
    ![라이선스 등록](../assets/images/install-guide-glue-license3-01.png){ align=center }
    - 구축 전, 발급 받은 **트라이얼 라이선스 파일** 을 등록 해줍니다.

2. 라이선스 확인
    ![라이선스 확인](../assets/images/install-guide-glue-license3-02.png){ align=center }
    - 등록된 라이선스 정보를 확인할 수 있는 화면입니다.
...
!!! info
    1번, 2번, 3번 호스트의 스토리지 센터 가상머신 생성이 마무리 되였습니다. ABLESTACK Glue 대쉬보드 사용을 위한 절차를 진행 후 Glue 대시보드를 이용하여 스토리지 센터 클러스터를 구성하시면 됩니다.
...
2. Glue 대시보드 접속 및 비밀번호 변경
    ...
    - 초기 비밀번호 변경하는 화면입니다.
    - **Old password** 입력창에는 기존의 비밀번호를 입력하고 **New password** 와 **Confirm new password** 입력창에 신규 비밀번호를 입력한 다음음 **Change Password** 버튼을 클릭합니다.
