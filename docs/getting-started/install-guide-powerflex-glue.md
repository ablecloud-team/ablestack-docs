...
5. 시간서버
    ...
    !!! info
        PowerFlex에서 시간서버는 매우 중요한 역할을 합니다. </br>
        ...
        폐쇄적인 네트워크 환경으로 외부 공인된 시간서버와 통신이 불가하고 내부에 별도의 시간서버가 없을 경우에는 PowerFlex에서 자체적으로 시간서버를 구성합니다. 이때에는 "로컬 시간서버"를 선택하여 진행하면 됩니다.
