...
!!! note "IPMI IPv4 Settings"
    - ABLESTACK은 호스트의 상태를 IPMI로 확인하므로, 클러스터 구성 후에도 지속적으로 호스트의 상태를 확인할 수 있도록 고정된 IPMI 주소 설정이 필요합니다.
    ![ABLESTACK IPMI 화면](../assets/images/install-ipmi-ipv4.png){ align=center }
!!! note "IPMI over LAN"
    - Mold 와 호스트간 IPMI 통신은 http/https 로 이루어지므로 'IPMI over LAN' 기능을 활성화해야 합니다.
    ![ABLESTACK IPMI 화면](../assets/images/install-ipmi-overlan.png){ align=center }
