...
HA(High Availability, 고가용성)는 장애가 발생했을 때 서비스 중단을 최소화하기 위한 기능입니다.
물리 서버(Cube)에 장애가 발생하면 해당 서버에서 실행 중이던 가상머신을 다른 정상 서버에서 자동으로 다시 실행합니다. 이를 통해 사용자는 서버 장애 상황에서도 서비스를 보다 안정적으로 운영할 수 있습니다.
다만 HA는 완전한 무중단 기능은 아니며, 가상머신이 다른 서버에서 재시작되는 동안 짧은 서비스 중단이 발생할 수 있습니다.

HA 기능이 정상적으로 동작하려면 공유 스토리지, 네트워크, 클러스터 구성이 올바르게 준비되어 있어야 합니다.

확인 항목은 다음과 같습니다.

1. 공유 스토리지(Glue)의 Heartbeat 체크 설정 값이 **"true"** 인지 확인합니다.
2. 클러스터에서 HA 기능이 활성화되어 있는지 확인합니다.
3. 호스트(Cube)의 HA 활성화 여부와 HA 상태가 정상인지 확인합니다.
4. Cube 터미널에서 Heartbeat 값이 주기적으로 확인되는지 점검합니다.

---

!!! warning "주의사항"
    1. HA 기능이 필요하지 않은 환경에서는 불필요하게 활성화하지 않는 것이 좋습니다.
    2. 여러 클러스터를 사용하는 경우, HA 대상 클러스터와 연결된 공유 스토리지의 Heartbeat 체크 설정 값을 **"true"** 로 등록해야 합니다.
    3. 모든 공유 스토리지에 Heartbeat 체크를 활성화하면 확인 부하와 응답 지연으로 인해 의도하지 않은 HA 동작이 발생할 수 있으므로 필요한 공유 스토리지에만 설정하는 것이 좋습니다.
    4. 설정 후에는 해당 Cube의 터미널에서 Heartbeat 값이 정상적으로 주기 확인되는지 점검해야 합니다.

---

아래 화면과 같이 공유 스토리지, 클러스터, 호스트의 HA 관련 설정과 상태를 확인합니다.

![HA 확인 화면1](../assets/images/checklist/checklist-ha-detail-1.png){ .imgCenter .imgBorder }
![HA 확인 화면2](../assets/images/checklist/checklist-ha-detail-2.png){ .imgCenter .imgBorder }
![HA 확인 화면3](../assets/images/checklist/checklist-ha-detail-3.png){ .imgCenter .imgBorder }
![HA 확인 화면4](../assets/images/checklist/checklist-ha-detail-4.png){ .imgCenter .imgBorder }
