...
# ABLESTACK VM Mold 설치진행

!!! danger
    이 문서는 기술지원 용도의 문서입니다. 기술지원 엔지니어가 아닌 사용자가 조작할 때 시스템에 문제가 발생할 수 있습니다.
    해당 설치과정에 사용되는 IP 및 입력 정보는 예시이며, 현장에 맞게 수정하시기 바랍니다.

ABLESTACK VM Mold 설치 진행 가이드 입니다.
이 문서에서는 ABLESTACK VM Mold 용 가상머신 생성 및 Mold 웹콘솔을 이용하여 Zone 구성까지 가이드 하고 있습니다.
ABLESTACK VM Cube 의 웹콘솔과 ABLESTACK VM Mold 웹콘솔을 이용하여 진행이 되며 웹 접속 IP는 별도의 표시를 하지 않고 진행됩니다.
...
!!! note
    ABLESTACK VM 클러스터는 1식의 이상 호스트로 구성되어야 합니다.
...
1. 개요
    ...
    - ABLESTACK VM를 구성을 하는데 필요한 정보를 입력 받아 클러스터 구성을 준비하는 마법사 화면입니다.
...
5. 시간서버
    ...
    !!! info
        ABLESTACK VM 시간서버는 매우 중요한 역할을 합니다. </br>
...
1. 개요
   ![클러스터 구성 준비 개요](../assets/images/install-guide-general-virtualization-mold-10.png){ .imgCenter .imgBorder }
    - 상단 리본의 **클러스터 구성 준비** 링크를 클릭하면 보이는 화면입니다.
    - ABLESTACK VM 구성을 하는데 필요한 정보를 입력 받아 클러스터 구성을 준비하는 마법사 화면입니다.
    - **다음** 버튼을 눌러 클러스터 구성 준비를 시작합니다.
...
## GFS 스토리지 구성
!!! warning
    GFS 스토리지 구성 전, 반드시 모든 서버에서 외부 스토리지 디스크와의 연결이 되었는지 확인해야 합니다.

![GFS 스토리지 구성](../assets/images/install-guide-general-virtualization-mold-gfs-storage-1.png){ .imgCenter .imgBorder }
- GFS 스토리지를 구성하기 위한 화면입니다. 상단 상태 리본의 GFS 스토리지 구성 링크를 클릭합나다.

1. 개요
    ![GFS 스토리지 구성 개요](../assets/images/install-guide-general-virtualization-mold-gfs-stroage-2.png){ .imgCenter .imgBorder }
    - GFS 스토리지 구성 개요 화면입니다. 개요의 내용을 확인 후 **다음** 버튼을 클릭합니다.

2. 외부 스토리지 동기화
    ![GFS 스토리지 구성 - 외부 스토리지 동기화](../assets/images/install-guide-general-virtualization-mold-gfs-stroage-3.png){ .imgCenter .imgBorder }
    - GFS 스토리지의 외부 스토리지 동기화 방식을 설정하는 화면입니다.
    - 이중화를 선택할 경우, 외부 스토리지 동기화 활성화 단계를 진행한 후, 다음 단계를 진행할 수 있습니다.
    - 외부 스토리지 동기화 활성화 버튼을 클릭합니다.
        ![외부 스토리지 동기화](../assets/images/install-guide-general-virtualization-mold-gfs-stroage-extenel-storage.png){ .imgCenter .imgBorder }

    !!! info
        이중화 - 외부 스토리지를 SAN 스위치를 통해 두 개 이상의 경로로 서버에 연결하고, 리눅스 커널의 multipath 서비스(MPIO)가 동일 LUN의 여러 경로를 하나의 논리 디스크로 묶어 관리합니다. 한 경로에 장애가 발생해도 다른 경로로 무중단(failover) 전환됩니다. 해당 단계는 외부 스토리지 동기화를 실행한 후, 다음 단계를 진행하셔야 합니다.

        단중화 - 외부 스토리지를 하나의 경로로만 연결하여 사용합니다. 구성은 간단하지만, 경로 장애시 디스크 접근이 즉시 중단될 수 있습니다. 테스트 환경이나 임시 구성에 적합합니다.

        건너뛰기 - 외부 스토리지 방식(이중화 또는 단중화) 구성을 마친 뒤 GFS 대상 디스크를 잘못 선택하셨다면, 외부 스토리지 동기화 단계에서 건너뛰기를 눌러 해당 단계를 건너뛸 수 있습니다.

3. GFS 디스크 구성
    ![GFS 스토리지 구성 - GFS 디스크 구성](../assets/images/install-guide-general-virtualization-mold-gfs-stroage-4.png){ .imgCenter .imgBorder }
    - GFS 디스크를 구성하는 화면입니다.
...
4. IPMI 정보
    ![GFS 스토리지 구성 - IPMI 정보](../assets/images/install-guide-general-virtualization-mold-gfs-stroage-5.png){ .imgCenter .imgBorder }
...
5. 설정확인
    ![설정확인](../assets/images/install-guide-general-virtualization-mold-gfs-stroage-6.png){ .imgCenter .imgBorder }
    - GFS 스토리지 구성전 설정을 확인하는 화면입니다.
    - 설정정보를 확인 후 **배포** 버튼을 클릭 합니다.

6. 구성
    ![구성](../assets/images/install-guide-general-virtualization-mold-gfs-stroage-7.png){ .imgCenter .imgBorder }
    - GFS 스토리지 구성 진행상황을 확인 할 수 있는 화면입니다.

7. 완료
    ![완료](../assets/images/install-guide-general-virtualization-mold-gfs-stroage-8.png){ .imgCenter .imgBorder }
    - GFS 디스크 구성이 완료 후 Cube 웹콘솔에서 GFS 리소스 상태 카드 항목에서 **펜스 장치 상태 및 잠금 장치 상태** 가 **Health Ok**,
      GFS 디스크 상태 카드에서 **마운트 경로** 가 존재하는지 확인하셔야 합니다.
...
10. 완료
    ...
    - 클라우드센터 가상머신 배포가 완료 후 ABLESTACK VM Cube 웹콘솔에서 클라우드센터 클러스터 상태 카드 항목에서 **클러스터 상태** 가 **Health Ok**,
...
11. 클라우드센터 기본스토리지 추가
    ...
    !!! tip
        경로 항목을 확인 할려면 ABLESTACK VM Cube 대시보드 화면에서 GFS 디스크 상태에서 마운트 경로를 확인하시면 됩니다.
...
!!! info
    ABLESTACK구성 시 필요한 최소 1식의 x86 호스트 서버가 준비되어 있어야 하며, 클라우드센터 가상머신은 총 1대입니다.

    클라우드센터 가상머신의 성능 안정화를 위해 **클라우드센터 가상머신(최소 16GB) + 여유 용량 (A)** 가 필요합니다.

### 호스트 고정 메모리 할당
1. 호스트 agent 구성파일
    ![호스트 설정파일](../assets/images/install-guide-general-virtualization-host-memory-add.png){ .imgCenter .imgBorder }
    - 호스트 Cube 터미널 화면입니다.
    - 터미널 명령어 **vi /etc/cloudstack/agent/agent.properties** 입력하고 Enter를 누릅니다.
    - **host.reserved.mem.mb=25600** 라인을 추가하고 **:wq** 명령어로 저장합니다.
